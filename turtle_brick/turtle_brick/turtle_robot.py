...
class TurtleRobot(Node):
    ...
    def __init__(self):
        ...
        self.goal_listener = self.create_subscription(
    ...
    def pose_callback(self, pose):
        """
        Turtle pose callback.

        Args:
        ----
        pose :
            Pose message from the turtlesim node
        ...
        if not self.setup_odom:
            ...
            world_odom_tf.transform.translation = Vector3(
                x=pose.x, y=pose.y, z=2 * self.wheel_radius + 0.4
            )
    ...
    def tilt_callback(self, tilt):
        """
        Tilt update callback.

        Args:
        ----
        tilt :
            Tilt message containing the current tilt angle
    ...
    def goal_callback(self, pose):
        """
        Goal pose update callback.

        Args:
        ----
        pose :
            PoseStamped message containing the goal location
    ...
    def timer_callback(self):
        ...
        if self.setup_odom:
            ...
            odom_bot_tf.transform.translation = Vector3(
                x=self.pose.x - self.startloc[0],
                y=self.pose.y - self.startloc[1],
                z=0.0,
            )
            ...
            if goalDist > 0.1:
                ...
                vectorToGoal = (
                    vel * (self.goalloc[0] - self.pose.x) / goalDist,
                    vel * (self.goalloc[1] - self.pose.y) / goalDist,
                )
...
def distBetweeinPoints(pointA, pointB):
    ...
    ----
    pointA :
        (x,y) location of point A
    pointB :
        (x,y) location of point B
